...
- **Map `Promise<T>` to `JsPromise[T]`** - Promise-returning WebAPI methods use the native `JsPromise[T]` interop type.
...
- **Add fetch example** - JS-only async/await demo using `JsPromise::wait()` to fetch from user-entered URLs, with status, headers, and body display.
