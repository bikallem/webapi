import { ParsedEnum } from '../types.js';
import { toSnakeCase, toPascalCase } from '../utils.js';
...
export function emitEnum(enumDef: ParsedEnum): string {
    ...
    for (const value of enumDef.values) {
        const variantName = toVariantName(value);
        lines.push(`    ${typeName}::${variantName} => TJsValue::to_js("${value}")`);
    }
    lines.push('  }');
    lines.push('}');
    lines.push('');
    ...
    for (const value of enumDef.values) {
        const variantName = toVariantName(value);
        lines.push(`    ${typeName}::${variantName} => logger.write_string("${value}")`);
    }
    lines.push('  }');
    lines.push('}');
