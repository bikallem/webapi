...
# Run from the project root: bash webapi_trim/tests/cli_test.sh
#
set -euo pipefail
...
run_trim() {
  ...
  moon -C webapi_trim run . -- "$@" 2>/dev/null
}
...
echo "# --help flag"
set +e
(cd "$PROJECT_ROOT" && moon -C webapi_trim run . -- --help) > "$TMPDIR_TEST/help.out" 2>/dev/null
code=$?
set -e
...
echo "# -o flag"
(cd "$PROJECT_ROOT" && moon -C webapi_trim run . -- "$FIXTURES/test.wasm" --source "$FIXTURES/sample.mjs" -o "$TMPDIR_TEST/out.mjs") > "$TMPDIR_TEST/o_stdout.out" 2>/dev/null
...
(cd "$PROJECT_ROOT" && moon -C webapi_trim run . -- "$TMPDIR_TEST/test.wasm" --source "$FIXTURES/sample.mjs") > "$TMPDIR_TEST/default_stdout.out" 2>/dev/null
...
REAL_WASM="$PROJECT_ROOT/examples/_build/wasm-gc/release/build/dom/dom.wasm"
REAL_SOURCE="$PROJECT_ROOT/webapi/webapi.mjs"
if [ -f "$REAL_WASM" ] && [ -f "$REAL_SOURCE" ]; then
  (cd "$PROJECT_ROOT" && moon -C webapi_trim run . -- "$REAL_WASM" --source "$REAL_SOURCE" -o "$TMPDIR_TEST/real.mjs") > "$TMPDIR_TEST/real_stdout.out" 2>/dev/null
