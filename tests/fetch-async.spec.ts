...
// Fetch-async example is JS-only.
const TARGETS = ['js'] as const;

for (const target of TARGETS) {
  test.describe(`fetch-async (${target})`, () => {
    test.beforeEach(async ({ page }) => {
      ...
      await page.goto(`fetch-async/fetch-async.${target}.html`);
    ...
    test('fetches default URL', async ({ page }) => {
      ...
      await expect(page.locator('#headers')).toContainText('application/json');
